...
import { ProductsAPI, type ProductListParams } from "../backend/apis/product.api"
...
export const productsKey = {
  ...
  top: (limit: number) => [...productsKey.root, "top", limit] as const,
}
...
export function useTopProductsQuery(limit = 10) {
  return useQuery<ProductDetail[]>({
    queryKey: productsKey.top(limit),
    queryFn: async () => unwrapApiCall(await ProductsAPI.top(limit)),
  })
}
